...
export default function Navbar() {
  ...
  const isActive = (path) => {
    return location.pathname === path ? 'bg-indigo-700 text-white shadow-inner' : 'text-slate-600 hover:bg-slate-100 hover:text-slate-900';
  };

  return (
    <header className="max-w-6xl mx-auto mb-8 flex flex-col md:flex-row items-center justify-between bg-white px-6 py-4 rounded-2xl shadow-sm border border-slate-100">
      ...
      <nav className="flex items-center gap-2 bg-slate-50 p-1.5 rounded-xl border border-slate-200">
        ...
        <Link to="/user" className={`px-4 py-2 rounded-lg text-sm font-semibold transition-all duration-200 ${isActive('/user')}`}>
          User Page
        </Link>
        <Link to="/queue" className={`px-4 py-2 rounded-lg text-sm font-semibold transition-all duration-200 ${isActive('/queue')}`}>
          Queue Dashboard
        </Link>
